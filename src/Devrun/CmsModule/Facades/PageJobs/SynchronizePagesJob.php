 ...
 *
 * @package Devrun\CmsModule\Facades\PageJobs
 */
class SynchronizePagesJob
{
    ...
    /**
     * SynchronizePagesJob constructor.
     *
     ...
     */
    public function __construct(EntityManager $entityManager, ModuleFacade $moduleFacade, Translator $translator, Container $context)
    {
    ...
    public function synchronizePages($need = false)
    {
        if ($pages = $this->moduleFacade->findUnSyncedPublicStaticPages($need)) {
            $this->synchronizationPages($pages);
            $this->synchronizationSections($pages);
        }
    }
